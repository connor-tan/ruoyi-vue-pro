package cn.iocoder.yudao.module.edu.controller.admin.publicationtype;
...
@Tag(name = "管理后台 - 刊物类型")
@RestController
@RequestMapping("/edu/publication-type")
@Validated
public class ProductPublicationTypeController {
    ...
    @PreAuthorize("@ss.hasPermission('edu:publication-type:create')")
    ...
    @PreAuthorize("@ss.hasPermission('edu:publication-type:update')")
    ...
    @PreAuthorize("@ss.hasPermission('edu:publication-type:delete')")
    ...
    @GetMapping("/get")
    @Operation(summary = "获取刊物类型")
    @Parameter(name = "id", required = true)
    @PreAuthorize("@ss.hasPermission('edu:publication-type:query')")
    public CommonResult<ProductPublicationTypeRespVO> get(@RequestParam("id") Long id) {
        return success(publicationTypeService.get(id));
    }

    @GetMapping("/page")
    @Operation(summary = "刊物类型分页")
    @PreAuthorize("@ss.hasPermission('edu:publication-type:query')")
    public CommonResult<PageResult<ProductPublicationTypeRespVO>> page(@Valid ProductPublicationTypePageReqVO reqVO) {
        return success(publicationTypeService.getPage(reqVO));
    }

    @GetMapping("/simple-list")
    @Operation(summary = "刊物类型精简列表")
    @PreAuthorize("@ss.hasPermission('edu:publication-type:query')")
    public CommonResult<List<ProductPublicationTypeSimpleRespVO>> simpleList() {
