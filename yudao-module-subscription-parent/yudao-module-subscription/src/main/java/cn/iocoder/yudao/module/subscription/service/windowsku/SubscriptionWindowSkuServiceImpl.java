...
public class SubscriptionWindowSkuServiceImpl implements SubscriptionWindowSkuService {
    ...
    public List<SubscriptionWindowSkuRespVO> getWindowSkuListByWindowSpuId(Long windowSpuId) {
        ...
        SubscriptionWindowDO window = subscriptionWindowService.getWindowDO(windowSpu.getWindowId());
        syncMissingWindowSkus(windowSpu, window.getTargetPeriod());
        ...
        return windowSkus.stream()
                .map(windowSku -> buildResp(windowSku, productSkuMap.get(windowSku.getProductSkuId()),
                        skuPublicationMap.get(windowSku.getProductSkuId()), window.getTargetPeriod()))
                .toList();
    }
    ...
    public void batchUpdateWindowSku(SubscriptionWindowSkuBatchUpdateReqVO reqVO) {
        ...
        Map<Long, ProductSkuPublicationDO> skuPublicationMap = subscriptionSupportService.getSkuPublicationMap(
                CollectionUtils.convertSet(windowSkuMap.values(), SubscriptionWindowSkuDO::getProductSkuId));
        for (SubscriptionWindowSkuBatchUpdateReqVO.Item item : reqVO.getItems()) {
            ...
            boolean targetPeriodMatched = SubscriptionSkuPeriodUtils.isMatched(
                    skuPublicationMap.get(windowSku.getProductSkuId()), window.getTargetPeriod());
            if (CommonStatusEnum.isEnable(item.getStatus()) && !targetPeriodMatched
                    && !CommonStatusEnum.isEnable(windowSku.getStatus())) {
                throw exception(ErrorCodeConstants.WINDOW_SKU_TARGET_PERIOD_NOT_MATCHED);
            }
    ...
    private void syncMissingWindowSkus(SubscriptionWindowSpuDO windowSpu, String windowTargetPeriod) {
        ...
        List<ProductSkuDO> productSkus = new ArrayList<>(subscriptionSupportService.getSkuListBySpuId(windowSpu.getProductSpuId()));
        Map<Long, ProductSkuPublicationDO> skuPublicationMap = subscriptionSupportService.getSkuPublicationMap(
    ...
    private SubscriptionWindowSkuRespVO buildResp(SubscriptionWindowSkuDO windowSku,
        ...
        respVO.setProductSkuTargetPeriod(SubscriptionSkuPeriodUtils.normalizeSkuTargetPeriod(skuPublication));
        respVO.setWindowTargetPeriod(SubscriptionSkuPeriodUtils.normalizeWindowTargetPeriod(windowTargetPeriod));
        boolean targetPeriodMatched = SubscriptionSkuPeriodUtils.isMatched(skuPublication, windowTargetPeriod);
        respVO.setTargetPeriodMatched(targetPeriodMatched);
        respVO.setDisabledReason(targetPeriodMatched ? null : "SKU 适用周期与窗口周期不一致");
        return respVO;
    }
}
