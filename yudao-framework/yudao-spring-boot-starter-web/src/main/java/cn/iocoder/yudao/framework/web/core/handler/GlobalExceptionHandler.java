...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        String tableName = extractMissingTableName(message);
        // 1. 数据报表
        if (isMissingTableOfModule(tableName, "report_")) {
        ...
        // 2. 工作流
        if (isMissingTableOfModule(tableName, "bpm_")) {
        ...
        // 3. 微信公众号
        if (isMissingTableOfModule(tableName, "mp_")) {
        ...
        // 4. 教育模块
        if (isMissingTableOfModule(tableName, "edu_student_promotion_")) {
            log.error("[教育模块 yudao-module-edu - 升班表结构未导入][请执行 sql/mysql/20260330_edu_student_promotion.sql 和 sql/mysql/20260330_edu_student_promotion_task.sql]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[教育模块 yudao-module-edu - 升班表结构未导入][请执行 sql/mysql/20260330_edu_student_promotion.sql 和 sql/mysql/20260330_edu_student_promotion_task.sql]");
        }
        // 5. 商城系统
        if (isMissingTableOfModule(tableName, "product_", "promotion_", "trade_")) {
        ...
        // 6. ERP 系统
        if (isMissingTableOfModule(tableName, "erp_")) {
        ...
        // 7. CRM 系统
        if (isMissingTableOfModule(tableName, "crm_")) {
        ...
        // 8. 支付平台
        if (isMissingTableOfModule(tableName, "pay_")) {
        ...
        // 9. AI 大模型
        if (isMissingTableOfModule(tableName, "ai_")) {
        ...
        // 10. IoT 物联网
        if (isMissingTableOfModule(tableName, "iot_")) {
    ...
    private String extractMissingTableName(String message) {
        String tableReference = StrUtil.subBetween(message, "'", "'");
        if (StrUtil.isBlank(tableReference)) {
            return null;
        }
        return StrUtil.subAfter(tableReference, ".", true);
    }

    private boolean isMissingTableOfModule(String tableName, String... prefixes) {
        if (StrUtil.isBlank(tableName)) {
            return false;
        }
        for (String prefix : prefixes) {
            if (StrUtil.startWith(tableName, prefix)) {
                return true;
            }
        }
        return false;
